...
class CustomExpr:
    ...
    def custom_wkb_to_cells(
        ...
        return self._expr.map_batches(lambda s: wkb_to_cells(s, resolution, containment_mode, compact, flatten))

    def custom_cells_to_wkb_polygons(self, radians: bool = False, link_cells: bool = False) -> pl.Expr:
        return self._expr.map_batches(lambda s: cells_to_wkb_polygons(s, radians, link_cells))

    def custom_from_wkb(self) -> pl.Expr:
        return self._expr.map_batches(lambda s: from_wkb(s))

    def custom_raster_to_dataframe(
        self,
        in_raster,
        transform,
        h3_resolution: int,
        ...
        return self._expr.map_batches(
            lambda s: raster_to_dataframe(s, in_raster, transform, h3_resolution, no_data_value, axis_order, compact)
        )
